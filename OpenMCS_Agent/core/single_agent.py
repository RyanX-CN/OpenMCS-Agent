...
The framework uses a plugin architecture:
...
- The framework loads and manages these plugins dynamically at runtime.
...
def build_single_agent(config_name=None):
    """构建并返回配置好的 Agent 实例"""
    # 获取配置字典
    ...
    # 确保 provider 是有效的（例如 'deepseek', 'openai' 等）
